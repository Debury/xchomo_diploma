...
if TYPE_CHECKING:
    # Only imported for type-checkers; importing at runtime would create a
    # circular import with src.sources, which dispatches to this module when
    # APP_DATABASE_URL is set.
...
def _to_dto(row: Source) -> "ClimateDataSource":
    ...
    ClimateDataSource is imported lazily here to avoid a circular import with
    src.sources (which lazily selects this store as its backend when
    APP_DATABASE_URL is set).
    """
    from src.sources import ClimateDataSource
